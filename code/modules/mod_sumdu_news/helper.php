...
/**
 * Helper for mod_sumdu_news
 *
 * @since  1.6
 */
abstract class ModSumduNewsHelper
{
	/**
	...
	public static function getList(&$params)
	{
		...
		// Set Days Limit
		$limit_days = $params->get('limit_days', 365);

		if (trim($limit_days) != '0') {
			$date_obj = new \DateTime('now');
			$date_obj->modify('-' . $limit_days . ' day');
			$search_date =  $date_obj->format('Y-m-d H:i:s');

			$model->setState('filter.date_filtering', 'relative');
			$model->setState('filter.date_field', 'a.created');
			$model->setState('filter.relative_date', $limit_days);
		}
		...
		foreach ($items as &$item)
		{
			...
			$item->introtext = JHtml::_('content.prepare', $item->introtext, '', 'mod_ssu_articles.content');
